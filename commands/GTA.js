...
var j = request.jar(); //Allows the gtaURL to remember the username and password
request = request.defaults({ jar: j }); // signs in to gtaURL
	request({ // logs in to gtaURL
		url: gtaURL,
		method: "GET",
		form: { UNENTRY: process.env.SOCIAL_USER, PWENTRY: process.env.SOCIAL_PASSWORD } //Provides Username and Password
	},
		function(error, response, body) {
			if (!error && response.statusCode == 200) {
				//console.log('body1:', body); 

//----------BEGIN Phantom Function to read and format html--------//
	(async function() {
		...
		let gtaTitles01 = gtaString.split("<p><b>"); //gets the titles
...
//------------------BEGIN capitalization Function-----------------//
	...
	//returns first letter capitalized + rest of the word lowercase if the word is the first word in the title - unless GTA
					if ( (Titles2[j] === Titles2[0]) && (!Titles2[j].includes("GTA")) ) { 
						gtaTitleString += `${Titles2[j].charAt(0)}${Titles2[j].toLowerCase().slice(1)} `; 
					}
	//returns all caps if title is GTA, GTA$, or XP							
					else if ( (Titles2[j].includes("GTA")) || (Titles2[j] === "XP") || (Titles2[j] === "RP") || (Titles2[j] === "GT")  ) { 
...
// 2-4: Adds only the title to gtaFinalString if no links, trailing <div>'s, or lists
				...
				if (gtaParaCount >= 3) { // if more than one paragraph, returns the second paragraph
					...
					if (gtaParas[0].includes("Luxury Autos Showroom")) { 
							//console.log(`3: gtaParas at I - ${i}: ${gtaParas[1]}\n`);
						gtaFinalString += `**${gtaTitles}**\n • ${gtaParas[0]}`; 
					} 
					//-----END paragraph count-----//	
					else { 
							//console.log(`4: Only Title at ${i}: ${gtaTitles}`);
						gtaFinalString += `**${gtaTitles}**\n\n\n\n\n\n`;
					}
				}
			}
...
//8: if gtaBonuses[0] contains lists but no links
				...
				if (gtaBonuses05[0].includes("Luxury Autos")) {
...
//---------------------------END populating gtaFinalString-------------------------//
	...
	// Below this differs from RDO 	
				...
				.replace(/Prime Gaming and sign up/g, "[Prime Gaming](https://gaming.amazon.com/loot/gtaonline)")
		...
		const aHour = aDate.getHours(); //Time of Day UTC (+6 MST; +4 EST)
 		...
 		let gtaExpiredEmbed = new MessageEmbed()
			.setColor('#00CD06') //Green
			.setDescription(`The above bonuses & discounts may be expired. \nRockstar updates these bonuses every Thursday after 12:30 PM EST`)

		await interaction.editReply({ embeds: [gtaImageEmbed, gtaEmbed] }).catch(console.error);
