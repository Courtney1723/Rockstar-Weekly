...
//-----END TRANSLATIONS-----//											
				...
				if (interaction.user.id === menuUserID) { //begin removing an gta channel
					...
					if ((interaction.user.id === process.env.USER_ID_1) || (interaction.user.id === process.env.USER_ID_2)) {
						console.log(`You removed ${menuChannelID} from receiving auto posts.`);
					}
					else {
						console.log(`a user removed ${menuChannelID} from receiving auto posts.`);
					}
				}
				else {
			...
			const expiredButton = new ActionRowBuilder()
				...
				setTimeout(() => {
					interaction.editReply({components: [expiredButton]}).catch(err => {console.log(`gtaRemove expiredButton Error: ${err.stack}`)});
				}, (60000 * 5))											
